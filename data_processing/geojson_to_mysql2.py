...
class contributions2(db.Model):
    ...
    SUB_ID = db.Column(db.String(19), unique=True, primary_key=True)
    ...
    def __repr__(self):
        return "<contributions2: {}>".format(self.__main__)
db.create_all()
...
for i in range(len(df)):
    ...
    TRANSACTION_AMT = float(df.loc[i, 'TRANSACTION_AMT']),
    ...
    try:
        db.session.add(post)
        db.session.commit()
        print('Success posting record for SUB_ID ' + str(df.loc[i, 'SUB_ID']))
    except:
        print('An error occurred posting record for SUB_ID ' + str(df.loc[i, 'SUB_ID']))
